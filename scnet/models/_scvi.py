...
from scnet.models._utils import sample_z
...
class scVI:
    ...
    def _encoder(self, name="encoder"):
        ...
        for idx, n_neuron in enumerate(self.architecture):
            if idx == 0:
                h = Dense(n_neuron, kernel_initializer=self.init_w, use_bias=False, name="first_layer")(self.x)
            else:
        ...
        model = Model(inputs=self.x, outputs=[mean, log_var, z], name=name)
    ...
    def to_latent(self, adata):
        ...
        latent = self.encoder_model.predict(adata.X)[2]
    ...
    def predict(self, adata, decoder_labels):
        """
        ...
        x_hat = self.model.predict([adata.X, decoder_labels])
    ...
    def restore_model(self):
        """
            ...
            ```
        """
        self.model = load_model(os.path.join(self.model_path, 'mmd_cvae.h5'), compile=False,
    ...
    def save_model(self):
        ...
        self.model.save(os.path.join(self.model_path, "mmd_cvae.h5"), overwrite=True)
    ...
    def train(self, train_adata, valid_adata, condition_key, cell_type_key='cell_type', le=None,
        ...
        if self.loss_fn in ['nb', 'zinb']:
            ...
            x_valid = [valid_adata.X, valid_conditions_onehot,
    ...
    def operate(self,
        ...
        new_network = scVI(**network_kwargs, **training_kwargs, print_summary=False)

        # Get Previous Model's weights
        used_bias_encoder = self.model.get_layer("encoder").get_layer("first_layer").use_bias
        used_bias_decoder = self.model.get_layer("decoder").get_layer("first_layer").use_bias

        if used_bias_decoder:
            prev_weights_decoder, prev_biases_decoder = self.model.get_layer("decoder").get_layer(
        ...
        if used_bias_decoder:
            new_network.model.get_layer("decoder").get_layer("first_layer").set_weights(
        ...
        for idx, encoder_layer in enumerate(new_network.encoder_model.layers):
            if encoder_layer.get_weights():
                encoder_layer.set_weights(self.encoder_model.layers[idx].get_weights())
        ...
        for idx, decoder_layer in enumerate(new_network.decoder_model.layers):
            if decoder_layer.name != 'first_layer' and decoder_layer.get_weights():
                decoder_layer.set_weights(self.decoder_model.layers[idx].get_weights())
        ...
        new_network.condition_encoder = self.condition_encoder
        for idx, new_condition in enumerate(new_conditions):
            new_network.condition_encoder[new_condition] = self.n_conditions + idx
